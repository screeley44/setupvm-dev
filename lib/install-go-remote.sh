  ...
  echo "else" >> rmt-go.sh
  echo "  cd ~" >> rmt-go.sh
  echo "  wget https://storage.googleapis.com/golang/go$GOVERSION.linux-amd64.tar.gz" >> rmt-go.sh
  echo "  rm -rf /usr/local/go" >> rmt-go.sh
  echo "  rm -rf /bin/go" >> rmt-go.sh		
  echo "  tar -C /usr/local -xzf go$GOVERSION.linux-amd64.tar.gz" >> rmt-go.sh
  echo "fi" >> rmt-go.sh
