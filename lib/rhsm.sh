  ...
  then
    ...
    then
      ...
      then
        ...
        until $SUDO subscription-manager repos --enable="rhel-7-server-rpms" --enable="rhel-7-server-extras-rpms" --enable="rhel-7-server-ose-$OCPVERSION-rpms" --enable="rhel-7-server-ansible-2.6-rpms" --enable="rhel-7-fast-datapath-rpms" --enable="rh-gluster-3-client-for-rhel-7-server-rpms" --enable="rhel-7-server-optional-rpms"; do echo "Failure Enabling Repos, retrying..."; sleep 8; done
        echo ""
      fi
