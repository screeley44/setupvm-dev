...
then  
  ...
  until $SUDO yum install wget git net-tools bind-utils iptables-services rpcbind nfs-utils glusterfs-client bridge-utils gcc python-virtualenv bash-completion telnet unzip kexec-tools sos psacct krb5-devel rpm-build createrepo bc rsync file jq tito gpgme gpgme-devel libassuan libassuan-devel -y> /dev/null; do echo "Failure installing utils Repos, retrying..."; sleep 8; done
